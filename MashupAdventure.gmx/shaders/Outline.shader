...
void main()
{
    ...
    // get alphas
    float sample = actual_color.a;
    sample = max(sample, texture2D( gm_BaseTexture, v_vTexcoord+offsetx ).a);
    sample = max(sample, texture2D( gm_BaseTexture, v_vTexcoord-offsetx ).a);
    sample = max(sample, texture2D( gm_BaseTexture, v_vTexcoord+offsety ).a);
    sample = max(sample, texture2D( gm_BaseTexture, v_vTexcoord-offsety ).a);
    
    if(sample > 0.0 && actual_color.a == 0.0){
        actual_color.rgb = Color;
        actual_color.a = sample;
    }
